...
def register_webrtc_events(socketio):
    ...
    @socketio.on('now-playing')
    def handle_now_playing(data):
        global current_now_playing
        print(f"[Server] Now playing update from {request.sid}: {data}")
        current_now_playing = data
        emit('now-playing', data, broadcast=True, include_self=False)
