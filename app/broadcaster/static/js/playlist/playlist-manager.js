...
import { renderPlaylist, clearPlaylistUI, updateNowPlaying, setUpPlaylist, setNowPlayingDeleteHandler } from './playlist-ui.js';
...
import { clearStreamAndPreview } from '../broadcaster/stream-manager.js';
...
class PlaylistManager {
  constructor() {
    ...
    setNowPlayingDeleteHandler(() => {
      if (this.core.currentIndex >= 0) {
        this.removeItemByIndex(this.core.currentIndex);
      }
      this.cleanupActiveMedia();
    });
    this.initUI();
  }

  cleanupActiveMedia() {
    window.currentActiveMedia?.pause?.();
    window.currentActiveMedia = null;
    stopAudioVisualizer();
    clearStreamAndPreview();
    const nowPlayingBlock = document.querySelector('.now-playing-block');
    nowPlayingBlock?.classList.remove('playing');
  ...
  initUI() {
    ...
    setUpPlaylist((newOrder) => this.reorderByFileIdOrder(newOrder));
    this.render();
  }
  ...
  removeItemByIndex(index) {
    const wasCurrentItem = index === this.core.currentIndex;
