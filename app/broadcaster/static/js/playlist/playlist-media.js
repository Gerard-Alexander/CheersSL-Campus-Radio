...
function updateAudioTrackTitle(title = 'No audio selected') {
  const titleElement = document.getElementById('audio-track-title');
  if (titleElement) {
    titleElement.textContent = title;
  }

  const streamManagerTitle = document.getElementById('stream-manager-title');
  if (streamManagerTitle) {
    streamManagerTitle.textContent = title;
  }
}

function formatTrackTitle(title = '') {
  return title.replace(/\.(mp3|wav|ogg|aac|flac|m4a|mp4|webm|avi|mov)$/i, '');
}

function broadcastNowPlaying(item) {
  const socket = window.socket;
  if (socket?.emit) {
    socket.emit('now-playing', {
      title: formatTrackTitle(item?.name || 'Unknown title'),
      url: item?.url || '',
...
export function playMediaItem(item, onEnd) {
  ...
  if (/(mp4|webm|avi|mov)/.test(ext)) {
    stopCurrentAudioDeck(() => {
      ...
      videoPreview.onplay = () => window.playlistControls?.setPlayPauseButtonState?.('playing');
      videoPreview.onpause = () => {
        if (window.currentActiveMedia === videoPreview) {
          window.playlistControls?.setPlayPauseButtonState?.('paused');
        }
      };
      ...
      updateAudioTrackTitle('No media selected');

      videoPreview.play().catch(err => console.warn('[play()] error:', err));
      updateNowPlaying(item);
      broadcastNowPlaying(item);
    });
    return;
  }
  ...
  if (/(mp3|wav|ogg|aac|flac|m4a)/.test(ext)) {
    ...
    nextDeck.onplay = () => window.playlistControls?.setPlayPauseButtonState?.('playing');
    nextDeck.onpause = () => {
      if (window.currentActiveMedia === nextDeck) {
        window.playlistControls?.setPlayPauseButtonState?.('paused');
      }
    };
    ...
    titleElement.className = 'audio-track-meta';
    const titleLabel = document.createElement('span');
    titleLabel.className = 'audio-track-label';
    titleLabel.textContent = 'Now Playing';
    const titleText = document.createElement('div');
    titleText.id = 'audio-track-title';
    titleText.className = 'audio-track-title';
    titleText.textContent = formatTrackTitle(item.name);
    titleElement.append(titleLabel, titleText);
