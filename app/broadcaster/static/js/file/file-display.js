...
import { playSideEffect, stopSideEffect } from './sfx-player.js';

export function displayInCard(container, name, url, uploadedBy = null, itemType = null) {
    ...
    media.title = itemType === 'sfx' ? 'Play as a background sound effect' : 'Add to playlist';

    media.addEventListener('click', () => {
        const ext = name.split('.').pop().toLowerCase();
        const isAudioFile = /(mp3|wav|ogg|aac|flac|m4a|mp2)/.test(ext);

        if (itemType === 'sfx' && isAudioFile) {
            stopSideEffect();
            playSideEffect(url, name);
            return;
        }
